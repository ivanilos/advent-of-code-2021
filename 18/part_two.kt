...
class SnailFishNumber() {
    ...
    override fun toString() : String {
        return if (isLiteralValue) {
            ...
            LEFT_DELIMITER + left!!.toString() + SEPARATOR +
                    right!!.toString() + RIGHT_DELIMITER
        }
    }
...
fun solve(snailFishNumbers : List<SnailFishNumber>) : Int {
    ...
    for (i in snailFishNumbers.indices) {
        for (j in snailFishNumbers.indices) {
            if (i != j) {
                val leftOperand = SnailFishNumber(snailFishNumbers[i].toString())
                val rightOperand = SnailFishNumber(snailFishNumbers[j].toString())
                val sum = leftOperand + rightOperand
                result = maxOf(result, sum.magnitude())
            }
        }
    }
    return result
