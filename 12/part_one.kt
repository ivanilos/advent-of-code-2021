...
class Graph(edges : List<List<String>>) {
    ...
    fun calculatePaths() : Int {
        val usedSmallCaves = mutableSetOf(start)

        return DFS(start, usedSmallCaves)
    }

    private fun DFS(node : Int, usedSmallCaves : MutableSet<Int>) : Int {
        if (node == end) return 1

        var result = 0
        for (adjNode in adjList[node]!!) {
            if (isSmallCave(adjNode) && adjNode !in usedSmallCaves) {
                usedSmallCaves.add(adjNode)
                result += DFS(adjNode, usedSmallCaves)
                usedSmallCaves.remove(adjNode)
